...
data$satisfaction <- factor(data$satisfaction, levels = c(0, 1))
...
(Giacomo) There are some NA values due to numerical instability (too many features) so we need to remove collinear features.
Based on the EDA, we know that some feature have no discriminative power like Gender, Age and Flight_Distance. 
From the correlation matrix then we know that Departure_Delay_in_Minutes and Arrival_Delay_in_Minutes are highly correlated so arbitrarly remove one of them.
By observing the glm output we can see that a lot of rating features have a really high p-value so we can remove them too because they are not statistically significant.
For example, Gate_location has a p-value greater than 0.99 and from the eda we know that it has no discriminative power.
Seat_comfort, Inflight_service, Cleanliness, Inflight_entertainment and On_board_service have a p-value greater than 0.95 and from the eda we can see that their distribution agains satisfaction are quite identical.
```{r}
# Model definition with all features:
glm_reduced<- glm(data = train_data, satisfaction ~ . - Departure_Delay_in_Minutes - Age - Flight_Distance - Gender
                                                    - Gate_location - Seat_comfort - Inflight_entertainment - On_board_service
                                                    - Inflight_service - Cleanliness - Checkin_service,
                family = "binomial")
# summary of full model
summary(glm_reduced)
```
...
```{r}
r2_full <- 1 - (summary(glm_full)$deviance/summary(glm_full)$null.deviance)
vif_full = 1/(1-r2_full)
r2_full
vif_full
```

```{r}
r2_reduced<- 1 - (summary(glm_reduced)$deviance/summary(glm_reduced)$null.deviance)
vif_reduced = 1/(1-r2_reduced)
r2_reduced
vif_reduced
```

(Giacomo) Comparing the R² sqaured values, we can see that dropping some features does not affect the model performance. 
This is a good sign because we can reduce the complexity of the model without losing performance.

Furthermore, the calculation of the variance inflation factor (VIF) is performed by taking the reciprocal of 1 minus the R² value. The VIF serves as a metric to evaluate the presence of multicollinearity, which refers to the correlation between predictor variables within a model. High levels of multicollinearity can introduce issues regarding the reliability and interpretability of the GLM (Generalized Linear Model).
Even here we can notice that the VIF values decreased meaning that the model has less collinearity between features.
...
vif_values
```
From the vif table we can see that some feature has a VIF value greater than 5. Thus we can proceed by removing one of them and check the VIF again.
...
# The process is done iteratively where we delete one variable at time
# Model definition:
glm_vif1<- update(glm_reduced, . ~ .-Ease_of_Online_booking, family = "binomial")
summary(glm_vif1)
vif_values <- VIF(glm_vif1)
vif_values
```
We can see that ratings_mean and onboard_rating_mean have a VIF value greater than 5. Thus we can say that the VIF spotted a probable collinearity relationship between these two features.

```{r}
# VIF Iteration 2
# The process is done iteratively where we delete one variable at time
# Model definition:
glm_vif2<- update(glm_vif1, . ~ . - onboard_rating_mean, family = "binomial")
summary(glm_vif1)
vif_values <- VIF(glm_vif2)
vif_values
```
We can notice that removing onboard_rating_mean the VIF of ratings_mean decreased. This is a good sign because we are reducing the collinearity between features.
Inflight_wifi_service still remained above 5 for all the iteration so we can proceed by removing it.
```{r}
# VIF Iteration 3
# The process is done iteratively where we delete one variable at time
# Model definition:
...
# Computing the predictions with the model on the test set:
pred_glm_backward<- predict(glm_vif3, data.frame(X_test), type = "response")
